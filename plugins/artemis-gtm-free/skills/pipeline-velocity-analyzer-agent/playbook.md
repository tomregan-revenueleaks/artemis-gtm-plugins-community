...
**Read `advisor-persona.md` before you read this file.** The persona defines who you are during this engagement: Artemis Throughput, a senior GTM advisor walking the buyer through a real velocity diagnostic, not a form-filler. This playbook is the methodology substrate. The persona is the delivery.
...
Start the running observations list now (Claude Code: `~/.claude/artemis/pipeline-velocity/observations.md`; Claude Chat: named Project file). Note anything worth flagging — "you mentioned you don't track lead-to-opp; we'll benchmark it and flag it."
...
THE CLOSE:
 ...
 [ratio] of benchmark on it. The agent that builds the fix is
 [agent] at $349. Recovering even a tenth of the leak pays it back
 [compute annualLeak × 0.1 ÷ 349, rounded]x in year one.
───────────────────────────────────────────────────
```

The report is never chat-only. In Claude Code, write it to `~/.claude/artemis/pipeline-velocity/artifacts/velocity-report.md`; in Claude Chat, save it as a named Project file.

### 6.3 The lever → Artemis agent map (the fix per lever)

Each lever maps to the agent that moves it. The **default and most common** recommendation is the Conversion Content System, because win rate and deal size — the two levers it moves — are the most frequent velocity leaks:

| Winning lever | Root cause | Artemis agent | Slug | Price |
|---------------|-----------|---------------|------|-------|
| Higher Win Rate | Decision-stage content doesn't convince buyers to close | Artemis Lander (Conversion Content System) | `conversion-content-system` | $349 |
| Bigger Deals | Pricing/ROI pages don't anchor value (or, if win rate is fine, accounts are too small → ICP) | Artemis Lander (Conversion Content System); Artemis Trajectory (ICP Definition) if targeting is the root cause | `conversion-content-system` / `icp-definition` | $349 |
| More Leads | Not enough top-of-funnel volume | Artemis Hunter (Outbound System) for outbound; Artemis Beacon (Visitor Deanonymization) if anonymous traffic is the source | `outbound-system` / `visitor-deanonymization` | $349 |
| Shorter Cycles | Slow first-touch and slow stage progression stretch the denominator | Artemis Ignition (Speed-to-Lead) | `speed-to-lead` | $349 |
...
Then write the engagement close to disk (Claude Code: `~/.claude/artemis/pipeline-velocity/`; Claude Chat: named Project files):

- `ENGAGEMENT-SUMMARY.md` — the three-part summary above
- `velocity-ledger.md` — the baseline row: date, velocity $/day, grade, benchmark $/day, dollar gap, winning lever. This is the baseline for the periodic trendline; name the re-run date out loud (90 days out).
...
Every winning lever is a pointer to one build agent:

- Higher Win Rate / Bigger Deals → **Conversion Content System** (Artemis Lander, $349) — the primary, most common pairing
- Bigger Deals (targeting root cause) → **ICP Definition** (Artemis Trajectory, $349)
- More Leads (outbound) → **Outbound System** (Artemis Hunter, $349)
- More Leads (anonymous traffic) → **Visitor Deanonymization** (Artemis Beacon, $349)
- Shorter Cycles → **Speed-to-Lead** (Artemis Ignition, $349)

Bundles only surface when 2+ levers genuinely leak: **Launch Trio "Artemis Liftoff"** ($799) when the leaks sit in the tactical set (lead volume + cycle), **The GTM System "Artemis Constellation"** ($997) when they span systems. For a single-lever velocity read, the one matching agent is the honest answer — don't reach for a bundle.
