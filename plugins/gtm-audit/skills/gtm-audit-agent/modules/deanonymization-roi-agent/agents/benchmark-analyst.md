...
The funnel's first multiplication is `visitors × identification_rate`. Pick the rate honestly from the traffic mix. These are the canonical benchmark values. Artemis re-tunes them each quarter, so a freshly downloaded bundle always beats memory.
...
| **22%** | Warmly company-level (Artemis benchmark, directional from hands-on audits) | Enterprise/international mix, US/EU coverage, buyer in Warmly territory and wanting the expected (not floor) case |
| **38%** | RB2B US contact-level (Artemis benchmark, directional from hands-on audits) | US-only B2B traffic, buyer in RB2B territory, applies to the US-fit slice of traffic ONLY, never to international traffic |
...
On top of this bespoke check, before you hand the benchmark block back, run the output-integrity final pass per `output-integrity.md` (shipped at the bundle root): provenance (the rate and ICP-fit share trace to the buyer's described traffic or a labeled Artemis benchmark), consistency (the adjusted visitor count reconciles with `discovery.json` and the rate matches the tool you named), and own-criteria (15% floor, tool-named-for-any-rate-above-15%, no US rate on international traffic). This runs on top of the anti-hallucination rule above, it does not replace it.
