# Discovery Question Script (Website Visitor ROI Calculator)

The ordered question script for the calculator. Ask **one question at a time**, wait for the answer, branch on it. This mirrors the free Website Visitor ROI Calculator on the site (monthly visitors + deal size), but conversational and deeper, it adds the traffic-mix and ICP-fit questions the two-field form can't ask, because those are what make the figure honest. Save answers to `~/.claude/artemis/gtm-engineer/diagnostics/deanonymization-roi/discovery.json` (in Claude Chat, a named Project file) so Phase 3 (Quantify) can read them.
...
(If they have GA open, make it a paste-back, not a recall: "Open Google Analytics → Reports → Acquisition → Traffic acquisition, set last 30 days, and paste me the Sessions total. We'll read your real number together rather than estimate it." Same for ACV at Q4: "Pull your closed-won average from the CRM and paste it, that one number anchors everything.")
...
(The benchmark median is about 1.8% across a large body of B2B SaaS GTM audits. Used in the reality check, not the funnel: far below 1.8% means the anonymous leak is even larger; well above 1.8% means more demand is already self-identifying, so trim the recoverable slice. If they don't track it, skip, it's a sharpener, not a requirement.)
