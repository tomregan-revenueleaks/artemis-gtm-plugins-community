...
2. **Only use numbers the buyer gave you.** If a value is missing, use the benchmark and SAY you did (12% for a missing conversion rate; the Artemis audit-set deal/lead context, drawn across a large body of B2B SaaS GTM audits, only where they truly have nothing). Never invent a deal size or a lead volume.
...
Before you hand the numbers back, run the output-integrity final pass (`output-integrity.md`, shipped at bundle root) on top of the sanity checks above: provenance (every figure traces to a buyer input or a labeled benchmark), consistency (the cost-of-delay tiles reconcile with the annual figure and nothing contradicts the discovery inputs), and own-criteria (the back-solve obeyed the rules in this file, no double decay, the 100% cap respected). The sanity checks are the leak-specific residue; this pass runs over them.
