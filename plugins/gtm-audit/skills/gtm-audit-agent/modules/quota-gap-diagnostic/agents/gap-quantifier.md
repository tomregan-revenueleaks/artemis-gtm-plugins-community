...
On top of these bespoke checks, run the output-integrity final pass (`output-integrity.md`, shipped at bundle root; in Codex `output-integrity.md`) before you return: provenance (every figure traces to a buyer input or a labeled Artemis benchmark, every substitution named in `assumptions`, nothing invented), consistency (the gap reconciles with the coverage and `discovery.json`, the activity nets out current pipeline, the cost-of-delay tiles annualize back to the gap, units agree), and own-criteria (your output matches the contract block above and the conservative-estimate rules hold). Close with the one-line integrity verdict so the parent inherits a checked number, not a raw one.
