...
# Routine: Quarterly Stack Re-Grade
...
- **The initial grade is complete and on disk.** `~/.claude/artemis/gtm-engineer/diagnostics/stack-grader/` contains `discovery.json` (the baseline inventory) and `coverage-score-ledger.md` with the baseline row. Verify: `ls ~/.claude/artemis/gtm-engineer/diagnostics/stack-grader/coverage-score-ledger.md` returns the file and it contains the Baseline row. No baseline, no trendline, run the grade first.
...
**Degraded mode:** if no fresh inventory is available when the routine fires (no connected tool, no buyer response), do not fabricate a stack, carry forward last quarter's inventory labeled "carried forward, not re-inventoried," write the report to `~/.claude/artemis/gtm-engineer/diagnostics/stack-grader/artifacts/` anyway, and flag that the inventory needs a real refresh next session. If Slack/email isn't connected, skip the push and surface the report at the next session open.

## Prompt to Claude (give this to `/schedule`, or to the OS-scheduler fallback)

```
You are running the quarterly Sales Tech Stack re-grade for [Company Name], comparing this quarter to the last recorded grade. Read the prior state from ~/.claude/artemis/gtm-engineer/diagnostics/stack-grader/ (discovery.json for the baseline inventory, coverage-score-ledger.md for the trendline so far).

BEFORE you re-measure, read back the prediction recorded on the prior ledger row: the lever you recommended last cycle (the #1 gap or redundancy and the partner platform that fills it, plus the Tech Stack Audit Agent if it was surfaced) and the impact you projected (the $/year that fix should recover, or the score points it should add). Hold it. The whole grade is an estimate (gap = stage ARR x leak rate, redundancy = stage ARR x 0.004 x extra tools), and this read-back is what lets you score whether the estimate held.
...
5. RECOMMENDED NEXT MOVE
 ...
 - If gaps or redundancies remain, surface the Tech Stack Audit module (`modules/stack/`) as the engagement that sequences the fix; route per the module-gate doctrine, the shell places it on the roadmap and quotes the price (see playbook cross-sell; no bundle ladder, one paid module).
...
6. ESTIMATE-VS-ACTUAL (the calibration)
 - Reconcile the prediction you read back at the top against what actually happened. Three reads, in one line: did the buyer ACT on the recommendation (fill the gap or cut the redundancy, or buy the Tech Stack Audit Agent to sequence it)? Did the metric actually MOVE (the Coverage Score, the gap's $ stake, the redundancy waste)? And how does PROJECTED read against ACTUAL (the priced leak was real and the fix recovered roughly the predicted dollars, or it under/over-shot, or they never acted so it's untested)? Close with one sentence of advisor judgment. Keep it human and one line, e.g. "Projected ~$400K/year recovered if they filled Visitor ID; they shipped Warmly and the score climbed the 22 points, the leak estimate held." Or: "Predicted the duplicate enrichment tool was $80K/year of waste; they cut it, the drag came off, the redundancy estimate held." Or: "They didn't act on the Visitor ID rec this quarter, so the $400K estimate is still untested; carrying it forward."
...
Write the outputs to disk: append this quarter's row to ~/.claude/artemis/gtm-engineer/diagnostics/stack-grader/coverage-score-ledger.md (including the estimate-vs-actual calibration line and this quarter's fresh prediction for next cycle to score) and save the full trend report to ~/.claude/artemis/gtm-engineer/diagnostics/stack-grader/artifacts/regrade-[YYYY-Qn].md. Then post the trend summary to the buyer's preferred channel (Slack #revenue-ops or email) if one is connected; if not, the artifacts copy is the deliverable. Lead with the score trajectory and the headline: "[Company] stack coverage: [score]/100 ([+/-X] vs last quarter)."
```

## The ledger (the durable trendline)

The trend lives in one named file: **`~/.claude/artemis/gtm-engineer/diagnostics/stack-grader/coverage-score-ledger.md`**. The initial grade writes the Baseline row at close; this routine appends one row per quarter. The ledger is the source of truth, the trend is read from it, never re-derived from memory:

| Quarter | Coverage Score | Grade | Maturity | Gaps Open | Redundancies | Total $ Stake | Top Gap | Recommendation + projected impact | Estimate vs actual (last cycle's prediction) |
|---------|----------------|-------|----------|-----------|--------------|---------------|---------|-----------------------------------|----------------------------------------------|
| Baseline | | | | | | | | | (none yet, first row) |
| Q1 | | | | | | | | | |
| Q2 | | | | | | | | | |

The Baseline row fills the Recommendation + projected impact column (the lever recommended and the $/year or points projected) and leaves the calibration column empty (nothing to reconcile yet). Each quarterly row reconciles the PRIOR row's prediction in the last column, then states its OWN fresh recommendation and projected impact in the prediction column for the next cycle to score. Keep both columns to one advisor-voice line, no raw data dumps, no invented sample sizes, no confidence intervals.

A healthy trajectory: score climbing, gap count falling, redundancies falling, total $ stake shrinking, maturity tier moving up (Starter → Foundation → Growth → Enterprise), and the estimate-vs-actual column showing the priced leaks were real and the fixes recovered roughly what was projected.
