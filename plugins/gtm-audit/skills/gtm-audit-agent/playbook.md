...
**Read `advisor-persona.md` before you read this file.** The persona defines who you are during this engagement: Artemis, a senior GTM advisor walking the buyer through a real diagnostic, not a form-filler. This playbook is the methodology substrate. The persona is the delivery.
...
This is the top-of-funnel diagnostic. The deliverable is not a built system. It's the map: where you're leaking, how much, and exactly which build module plus which partner platform closes each leak. It is free: the top-of-funnel diagnostic that tells you which build modules you actually need before you spend a dollar guessing.
...
6. A prioritized roadmap, leak #1 first, then #2, each mapped to the build module that fixes it and the partner platform it runs on
...
Every confirmed leak (a below-0.7x metric, OR a structural gap like No-Tools or no scoring) maps to a named leak. For each leak below: how you detect it from their metrics, how you quantify the dollars, how it moves the health score, and which build module + partner platform fixes it. Route each module by plain name and directory (`modules/<system>/`), never a price in prose and never a checkout link. Only surface a leak when the detection condition is actually met by THEIR data. Never list a leak the inputs don't support.
...
- **Fixes:** Build module → the **Speed-to-Lead** module (`modules/speed-to-lead/`), routed per the module-gate doctrine (owned starts now, unowned gated honestly, price quoted at runtime from the module manifest, never in prose). Partner → **Amplemarket** (artemisgtm.ai) for the outbound sequence layer, **Warmly** (artemisgtm.ai) for the inbound trigger.
...
- **Fixes:** Build module → the **Visitor Deanonymization** module (`modules/visitor-id/`), routed per the module-gate doctrine (owned starts now, unowned gated honestly, no price in prose). Partner → **Warmly** (artemisgtm.ai) for richer GTM workflow, **RB2B** (artemisgtm.ai) for US-only narrow-budget cases.
...
- **Fixes:** Build module → the **Lead Scoring** module (`modules/scoring/`), routed per the module-gate doctrine (owned starts now, unowned gated honestly, no price in prose). Partner → **Amplemarket** / **Apollo** for enrichment-driven intent (artemisgtm.ai, artemisgtm.ai), **Warmly** for behavioral intent.
...
- **Fixes:** Build module → the **ICP Definition** module (`modules/icp/`), routed per the module-gate doctrine (owned starts now, unowned gated honestly, no price in prose). Partner → **Sybill** / **Attention** for mining discovery transcripts (artemisgtm.ai, artemisgtm.ai).
...
- **Fixes:** Build module → the **Tech Stack Audit** module (`modules/stack/`), routed per the module-gate doctrine (owned starts now, unowned gated honestly, no price in prose). Partner → the consolidation usually lands on **Amplemarket** (engagement + enrichment + intent in one) and **Warmly** (artemisgtm.ai, artemisgtm.ai).
...
- **Fixes:** Build module → the **Content System** module (`modules/content/`), routed per the module-gate doctrine (owned starts now, unowned gated honestly, no price in prose). Partner → **Amplemarket** for distribution amplification (artemisgtm.ai).
...
- **Fixes:** Build module → the **Outbound System** module (`modules/outbound/`), routed per the module-gate doctrine (owned starts now, unowned gated honestly, no price in prose). Partner → **Amplemarket** (sequencing + signals, artemisgtm.ai), **Maildoso** for sending infrastructure (artemisgtm.ai), **Warmly** for intent triggers.
...
- **Fixes:** Build module → the **Nurture System** module (`modules/nurture/`), routed per the module-gate doctrine (owned starts now, unowned gated honestly, no price in prose). Partner → **Amplemarket** (multi-channel re-engagement), **Attio** (CRM behavior triggers).
...
- **Fixes:** Build module → the **Conversion Content System** module (`modules/conversion/`), routed per the module-gate doctrine (owned starts now, unowned gated honestly, no price in prose). Partner → none required; this is a content/CRO build.
...
- **Fixes:** Build module → the **Qualification Automation System** module (`modules/qualification/`), routed per the module-gate doctrine (owned starts now, unowned gated honestly, no price in prose). Partner → **Amplemarket**, **Attio**, **Attention** (artemisgtm.ai).
...
- **Fixes:** Build module → the **AI RevOps System** module (`modules/revops/`), routed per the module-gate doctrine (owned starts now, unowned gated honestly, no price in prose). Partner → **Attio** (CRM), **Attention** / **Sybill** for call-data hygiene.
...
## Step 7: Recommend (the prioritized roadmap)

This is the payoff, and the close routes to the roadmap, never to a checkout. The audit's whole job is to end here: a ranked roadmap where every confirmed leak maps to the exact build module that fixes it and the partner platform it runs on.
...
PRIORITY 1, [Leak name] [Severity] [$X/year]
 ...
 The fix: [Build module name] module (modules/<system>/), routed
...
───────────────────────────────────────────────────
THE CLOSE:
 Start with Priority 1, that's the [module] module, fixing your biggest bleed.
 OR (3+ leaks): you've got [N] leaks worth [$X] combined. Here's the build
 order, leak #1 first, each mapped to the module that fixes it. When the
 leaks span motions, the combined build path fixes all of them; if you want
 to know what any module or the combined path costs, I'll quote it from the
 module manifest at that moment, no price guessing from me.
───────────────────────────────────────────────────
```
...
### 7.2 The recommendation logic (route the roadmap, quote prices only from the manifest)

- **One confirmed leak** → route the single matching module by plain name and directory (`modules/<system>/`). Name it, tie it to the leak's dollar figure. Do the return math out loud from their own number without inventing a price: "Your #1 leak is [X] at [$Y]/yr. The module that fixes it is the [Module] module; even recovering a fraction of that leak pays the build back many times over in year one." If the buyer asks what it costs, quote it at that moment from the module manifest and nowhere else.
- **Two or more confirmed leaks** → present the sequenced roadmap, leak #1 first, each mapped to its module. When the leaks span motions, name the combined build path as the option that fixes all of them in order (ICP Definition first when targeting is broken, then the tactical fixes, then the systems). Do NOT assemble a bundle price, a discount, or a savings figure in prose.
- **What any module or the combined path costs** comes from the module manifest at runtime, quoted once at the moment the buyer asks, never from this playbook and never from memory. The roadmap is the deliverable; the price is a runtime lookup, not prose.
- Because an audit usually surfaces 3+ leaks, the combined build path is the natural landing spot more often than not. Don't undersell it: if the roadmap genuinely needs three-plus systems, the sequenced build path IS the honest answer, not an upsell.
...
Always say what the tool is FOR in the context of this leak ("Warmly to capture the inbound visitor signal you're currently losing"), never just "use Warmly." Defer all pricing and feature questions to the partner's site, never quote a partner's price or plan tier. Never quote a specific audit count (no specific n= or dataset counts) or a named dataset figure in buyer output; use the hedged phrasing.
...
3. **What I'd fix first, and what to automate next.** Priority 1 with its routed module (owned starts now, unowned gated honestly), then the running observations list (the side-stuff you logged that didn't become a headline leak), prioritized by impact and effort.
...
- `ENGAGEMENT-SUMMARY.md`, the three-part summary above
 ...
 Logging the prediction here is what lets the quarterly re-run grade the call (concept borrowed from `results-memory.md`'s free-diagnostic note: a diagnostic that recommends a fix should learn whether the fix moved the number). You're recording, dated, the lever you recommended (which leak, which module) and the impact you projected ($/yr recovery), in the same `health-score-ledger.md` row, never a separate file. Write it in advisor voice, the way you framed it to the buyer ("recommended the Speed-to-Lead module for the response leak, projected ~$310K/yr recovered"), not as raw fields.
...
**Gotcha 4. A precise-looking fake number.** "$1,437,920/year" reads as invented. Fix: round to "$1.4M/year", use the conservative end of ranges, always show the calculation chain. Same rule on substantiation counts: never quote a specific audit count (no specific n= or dataset counts) or a named dataset figure in buyer output; use the hedged phrasing "across a large body of B2B SaaS GTM audits."
...
- [ ] Sequenced build roadmap presented when 2+ leaks surfaced; any module or combined-path price quoted only from the module manifest at runtime, never in prose
...
## Routes to (the build modules this audit points to)

This diagnostic's whole output is a map to the build modules. Every confirmed leak is a pointer, routed by plain name and directory:

- Slow response → **Speed-to-Lead** module (`modules/speed-to-lead/`)
- Anonymous traffic → **Visitor Deanonymization** module (`modules/visitor-id/`)
- No scoring → **Lead Scoring** module (`modules/scoring/`)
- Wrong ICP → **ICP Definition** module (`modules/icp/`)
- Tool sprawl → **Tech Stack Audit** module (`modules/stack/`)
- Content gap → **Content System** module (`modules/content/`)
- Outbound gap → **Outbound System** module (`modules/outbound/`)
- Nurture gap → **Nurture System** module (`modules/nurture/`)
- Conversion-surface gap → **Conversion Content System** module (`modules/conversion/`)
- MQL→SQL gap → **Qualification Automation System** module (`modules/qualification/`)
- RevOps gap → **AI RevOps System** module (`modules/revops/`)

Route each per the module-gate doctrine: an owned module (its directory is present) starts its build inside this same engagement with no purchase; an unowned one gets the honest gate at the close. From the second confirmed leak on, present the sequenced build roadmap and, when the leaks span motions, name the combined build path. Never write a module or bundle price in prose; if the buyer asks, quote it once from the module manifest at that moment.
