...
**0. Confirm I actually loaded (10 seconds).** There's no `/skills` command on the
web, so the one reliable check is the kickoff phrase: start a new chat and say the
engagement phrase. You'll know it worked when I answer in character and ask you a
discovery question, not when you get generic Claude. If you get generic Claude, the
skill didn't attach: check that **Code execution and file creation** and **Skills**
are on under **Settings, Capabilities** (on Team or Enterprise an admin enables them),
confirm the skill shows at **claude.ai/customize/skills**, then resend the phrase. Do
this once up front so we don't build a whole plan against a Claude that never had me.
...
**3. Memory is on by default, and it remembers your results, not just your answers.**
I'll remember your answers and decisions across sessions. More than that: once we
ship your system, I keep a running record of what it actually produces (reply rates,
meetings, which message won) so every recommendation I make gets sharper from your
real results instead of starting from generic defaults. You don't manage anything; I
update it after every check-in. Optional: drop your company facts into a `CLAUDE.md`
file in your project folder so they load every session too.
