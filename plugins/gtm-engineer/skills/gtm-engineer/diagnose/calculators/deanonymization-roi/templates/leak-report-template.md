# Visitor-Pipeline Report Template (Website Visitor ROI Calculator)
...
> "Built on your real visitor count and deal size, with the funnel rates at conservative benchmark defaults. The two numbers that move it most. ICP-fit traffic share and ACV, are yours, so this is a defensible estimate, not a guess."
...
> "About 98% of your visitors leave anonymous; the benchmark median form-fill rate is about 1.8% across a large body of B2B SaaS GTM audits. Nothing routes them today, so the in-market accounts researching you walk out unseen. The funnel above is what a conservative identification setup recovers."

**Reusable formula card** (so you can re-run this yourself any time your traffic changes):

```
ICP-fit visitors × ID rate × 20% high-intent × 8% account-meeting
 × 50% meeting-to-opp × ACV × 12 = annual pipeline
```

Meetings count at the account level. Swap in your real numbers and the figure updates, you don't need me to recompute it.
...
### The leak → module map (as shipped)

| Need | Artemis module | Module path |
|------|----------------|-------------|
| Anonymous visitors / website de-anon | Visitor Deanonymization | `modules/visitor-id/` |

Route per the module-gate doctrine: log the gap, the shell places it on the roadmap and quotes the price. Never a prose price, a checkout link, or a codename.
...
roughly $[small figure]/year, not enough to justify a build or a monthly tool.
...
the Visitor Deanonymization module (modules/visitor-id/) on [Warmly / RB2B] for your traffic.
...
hand back to the shell's full GTM diagnostic, which sizes leaks
