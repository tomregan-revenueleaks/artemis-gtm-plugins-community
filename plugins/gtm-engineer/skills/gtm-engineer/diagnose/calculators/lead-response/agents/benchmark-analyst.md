...
2. Where their response time sits against the Artemis median of 42 hours (drawn across a large body of B2B SaaS GTM audits)
...
(Decay curve from Oldroyd et al., 2011, "The Short Life of Online Sales Leads," HBR; InsideSales.com Lead Response Management Study, 2017. These are the exact `DEFAULT_MULTIPLIERS` the live calculator applies. They are the canonical shipped copy. Artemis re-tunes these benchmarks each quarter, so a freshly downloaded bundle always beats memory. If this file and the playbook ever disagree, the playbook wins.)

## The benchmark placements

- **Response time vs the 42-hour median.** The Artemis median response time, across a large body of B2B SaaS GTM audits, is 42 hours, which falls in the **24+ hours** tier (also the InsideSales.com average). State plainly where the buyer sits relative to it: faster than the median, slower, or right around it.
...
vs 42-hr median: [faster than / slower than / around] the Artemis audit-set median
...
Before you hand the read back, run the output-integrity final pass (`output-integrity.md`, shipped at bundle root) on top of the placement rules above: provenance (the tier, the grade, and the back-solved ceiling all trace to a buyer input or a labeled benchmark), consistency (the read contradicts nothing in the discovery inputs and the ceiling math reconciles), and own-criteria (the leak/no-leak flag follows the rule in this file and any benchmark-filled input is marked directional). The placement rules are the leak-specific residue; this pass runs over them.
