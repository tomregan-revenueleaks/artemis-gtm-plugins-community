...
→ **Setup-coaching beat (offer once, right here, after the first Discovery exchange).** This is the moment for the 60-second setup check from `claude-setup.md`. Offer it in one sentence, then respect the answer: "Before the numbers, I can take 60 seconds to make sure Claude is set up to give you the sharpest velocity read, or we can skip it." Record the outcome in `discovery.json` under `setup_check`. If it's already done or skipped, don't re-offer.

---

## Block 2: The Four Inputs

**Offer the artifact path first (the paste-back move).** Before you ask the four numbers one at a time, give the buyer the faster, more grounded option: "If you have a CRM funnel report or a pipeline dashboard handy, paste a screenshot or the raw numbers here and I'll pull leads, lead-to-opp, win rate, and cycle straight off it, then read them back for you to confirm. That keeps the velocity grounded in your actual data instead of recall." If they paste something, extract the four inputs, read each one back, and have them confirm before you compute, this converts the intake from recall to an artifact you react to. If they'd rather just answer, fall through to the questions below.
