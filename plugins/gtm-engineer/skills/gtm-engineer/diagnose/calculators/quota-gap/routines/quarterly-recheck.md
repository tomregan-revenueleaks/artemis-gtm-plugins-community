...
# Routine: Quarterly Quota Coverage Re-Check
...
**Degraded mode:** if no fresh numbers are available when the routine fires (no connected tool, no buyer response), do not fabricate a reading, carry forward last quarter's values labeled "carried forward, not re-measured," write the re-check to `~/.claude/artemis/gtm-engineer/diagnostics/quota-gap/artifacts/` anyway, and flag which inputs need real numbers next session. If Slack/email isn't connected, skip the push and surface the re-check at the next session open.

## Prompt to Claude (give this to `/schedule`, or to the OS-scheduler fallback)

```
You are running the quarterly quota coverage re-check for [Company / Team], comparing this quarter to the last recorded run. Read the prior state from ~/.claude/artemis/gtm-engineer/diagnostics/quota-gap/ (discovery.json for the baseline inputs, coverage-ledger.md for the trendline so far).

Before you re-measure anything, read the prior prediction recorded on the last ledger row: the lever that was diagnosed (volume / conversion / capacity), the ONE agent that was recommended for it, and the projected coverage gain. Hold that prediction so you can check it against what actually happened this quarter; the re-check is also the calibration of that earlier call.
...
5. PREDICTION CALIBRATION (reconcile the last call against reality)
 - Pull the prediction you read off the prior ledger row at the top of this run: the lever diagnosed, the ONE agent recommended, and the projected coverage gain.
 - Answer three things from this quarter's numbers: (a) did the buyer act on the recommendation, did they build or buy the recommended agent or fix? (b) did the metric actually move, is coverage up and the gap down, or flat? (c) the projected-versus-actual read, did coverage land in the band you projected, fall short, or overshoot?
 - Close with one sentence of advisor judgment on whether the original diagnosis held. Keep it human, one line. For example: "Projected coverage climbing toward 4.5x if they shipped the Outbound System; they built it and coverage moved from 1.8x to 3.4x as projected, the volume-leak call held." Or, honestly, if they did not act: "They didn't build the recommended Outbound System and coverage stayed at 1.8x; the volume diagnosis is untested, not wrong, re-recommend it."
...
Write the outputs to disk: append this quarter's row to ~/.claude/artemis/gtm-engineer/diagnostics/quota-gap/coverage-ledger.md, INCLUDING the one-line prediction calibration (did they act, did the metric move, projected vs actual, the one-sentence judgment) so the next run reads it back the same way you just did, and save the full trend report to ~/.claude/artemis/gtm-engineer/diagnostics/quota-gap/artifacts/recheck-[YYYY-Qn].md. Then post the trend summary to the buyer's preferred channel (Slack #revenue-ops or email) if one is connected; if not, the artifacts copy is the deliverable. Lead with the coverage trajectory and the headline: "[Company] quota coverage: [X.X]x ([+/-X.X]x vs last quarter), gap [$X] ([up/down] [$X])."
```

## The ledger (the durable trendline)

The trend lives in one named file: **`~/.claude/artemis/gtm-engineer/diagnostics/quota-gap/coverage-ledger.md`**. The initial diagnostic writes the Baseline row at close; this routine appends one row per quarter. The ledger is the source of truth, the trend is read from it, never re-derived from memory:

| Quarter | Annual Target | Win Rate | Coverage | Grade | Quarterly Gap | Primary Problem | Days to Cutoff | Prediction (lever + agent + projected gain) | Calibration (acted? moved? projected vs actual + judgment) |
|---------|---------------|----------|----------|-------|---------------|-----------------|----------------|----------------------------------------------|-------------------------------------------------------------|
| Baseline | | | | | | | | the call made at diagnosis: lever, recommended agent, projected coverage gain | (filled by the next re-run) |
| Q1 | | | | | | | | this quarter's call | did they act, did coverage move, projected vs actual, one-line judgment on last quarter's prediction |
| Q2 | | | | | | | | | |

The Prediction column holds the call made the moment the row was written (the lever diagnosed, the one agent recommended, the projected coverage gain). The Calibration column is filled by the FOLLOWING re-run, after it re-measures, reconciling that prior prediction against what actually happened. Keep each cell to one human line; no raw-data dumps, no invented precision.

A healthy trajectory: coverage ratio climbing toward (and past) the win-rate requirement, grade rising, the dollar gap shrinking, the diagnosis moving toward on-track, and the prediction column repeatedly calibrating "the call held."
