...
Before you return these numbers to the parent skill, run the output-integrity final pass (provenance, consistency vs `discovery.json`, own-criteria) per `output-integrity.md`, shipped at bundle root. This runs ON TOP of the sanity checks and anti-hallucination rules above, it does not replace them: confirm every figure traces to a buyer input or a labeled Artemis benchmark, that nothing contradicts Discovery, and that the per-lever split reconciles to the total. State the one-line integrity verdict alongside what you return.
