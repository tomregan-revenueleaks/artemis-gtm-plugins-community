# Discovery Question Script (DIAGNOSE mode: The AI GTM Engineer)

The ordered question script for the audit. Ask **one question at a time**, wait for the answer, branch on it. This mirrors the free flash-audit wizard (Company Profile → Revenue Metrics → Team & Growth → Signal-stack), but conversational and deeper. Save answers to `~/.claude/artemis/gtm-engineer/diagnostics/discovery.json` (in Claude Chat, a named Project file) so Diagnose (Step 5) can read them, and mirror the durable profile facts (company, domain, industry, size, motion, rough ARR band) to `~/.claude/artemis/_profile/profile.json`. This is the **one-discovery-forever** write: every build module later reads these from the profile and confirms deltas only, so the buyer never re-explains their company.
...
**Profile check first (never re-ask).** Before Q1, read `~/.claude/artemis/_profile/profile.json`. If it exists, confirm the durable facts in one line, "I have you as [company], a [size] [industry] company running a [motion] motion, still right?", and skip straight to whatever delta the profile is missing. Only cold-ask Q1 to Q4 below for facts the profile does not already hold; the buyer never re-explains their company.
