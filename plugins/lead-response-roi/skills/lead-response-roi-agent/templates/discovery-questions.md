# Discovery Question Script (Lead Response ROI — Artemis Stopwatch)

The ordered question script for the diagnostic. Ask **one question at a time**, wait for the answer, move to the next. This mirrors the free Lead Response ROI Calculator's four inputs (Monthly Leads → Deal Size → Response Time → Conversion Rate), but conversational and with the benchmark toggle on each. Save answers to `~/.claude/artemis/lead-response-roi/discovery.json` (in Claude Chat, a named Project file) so Diagnose (Step 4) can read them.
...
The Artemis 127-audit median is **42 hours** (the 24+ tier). If they truly can't say, offer the **4-24 hour** tier as a deliberately conservative placeholder (not the 24+ median, so an assumption doesn't inflate the leak) and tell them you're being conservative.

→ Save Q3. Place it against the median out loud: "42 hours is the median across the 127 audits; you're at [tier], which is [faster/slower/around] that." Then: "Last input, and it's the one people get wrong, so I'll be precise about it."
