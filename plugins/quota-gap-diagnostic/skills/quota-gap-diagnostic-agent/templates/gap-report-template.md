# Gap Report Template (Quota Gap Diagnostic — Artemis Quota Line)
...
═══════════════════════════════════════════════════
 ...
 Analyzed [Date] · Artemis Quota Line
═══════════════════════════════════════════════════
...
### The canonical diagnosis → Artemis agent map (slugs and prices as shipped; the live page at each link is authoritative if they ever differ)

| Primary problem | Artemis agent | Slug | Price | Partner |
|-----------------|---------------|------|-------|---------|
| Volume (under-covered, not enough pipeline) | Artemis Hunter — Outbound System | `outbound-system` | $349 | Amplemarket + Warmly |
| Conversion (win rate < 20%) | Artemis Lander — Conversion Content System | `conversion-content-system` | $349 | Attention |
| Capacity (AEs > 25 active opps) | Artemis Gateway — Qualification Automation System | `qualification-automation-system` | $349 | Amplemarket |
| On track (covered + healthy capacity + win rate ≥ 20%) | — (optional Lead Scoring to protect pipeline) | `lead-scoring` | $349 | Attention (optional) |
...
If you want to keep the edge as you scale, Lead Scoring (Artemis Vector,
$349, https://artemisgtm.ai/agents/lead-scoring/) routes the highest-fit
opps first so coverage quality holds. Optional, not a fix.
```
...
The agent that closes it is [Artemis agent] ([slug], $349). Start there; the
partner platform under it is [Tool].
